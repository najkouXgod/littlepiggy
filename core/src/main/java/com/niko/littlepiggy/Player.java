...
public class Player {
    ...
    private enum AnimationState { IDLE, RUNNING, CHARGING }
    ...
    public Player(World world, float startX, float startY, GameAssets assets) {
        ...
        footShape.setAsBox(sprite.getWidth()/2f * 0.5f, 0.1f,
            new Vector2(0, -sprite.getHeight()/2.5f), 0);
    ...
    private void handleInput() {
        ...
        float vx = 0;
        if (Gdx.input.isKeyPressed(Input.Keys.RIGHT)) { vx =  DebugConfig.SPEED; facingLeft = false; isMoving = true; }
        if (Gdx.input.isKeyPressed(Input.Keys.LEFT))  { vx = -DebugConfig.SPEED; facingLeft = true;  isMoving = true; }
    ...
    private void handleJump() {
        ...
        if (spacePressed) {
            ...
            if (spacePressedTime >= DebugConfig.TAP_THRESHOLD) {
                ...
                if (Math.abs(newVx) < 0.1f) newVx = 0f;
        ...
        if (!spacePressed && spaceWasPressed) {
            if (spacePressedTime < DebugConfig.TAP_THRESHOLD) {
                ...
                float jumpStrength    = DebugConfig.JUMP_MINPOWER + (DebugConfig.JUMP_MAXPOWER - DebugConfig.JUMP_MINPOWER) * curvedCharge;
